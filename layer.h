...
public:
	...
	Layer(int M, int N, int O, bool cpu);
	Layer(int M, int N, int O, FILE *weights_file);
	Layer(int M, int N, int O, FILE *weights_file, bool cpu);
...
void cpu_fp_preact_c1(float input[28][28],float preact[6][24][24], const float weight[6][5][5]);
