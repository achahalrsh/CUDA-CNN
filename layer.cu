...
// Constructor
Layer::Layer(int M, int N, int O, bool gpu_flag)
{
	this->M = M;
	this->N = N;
	this->O = O;
	this->is_gpu = gpu_flag;
...
Layer::Layer(int M, int N, int O, FILE *weights_file, bool gpu_flag)
{
	this->M = M;
	this->N = N;
	this->O = O;
...
void Layer::save(std::ofstream &weights_file)
{
	...
	if (is_gpu) {
		...
		memcpy(h_bias, bias, sizeof(float) * N);
		memcpy(h_weight, weight, sizeof(float) * M * N);
	}
...
void cpu_bp_output_s1(float d_output[6][6][6], float n_weight[10][6][6][6], float nd_preact[10]) {
	...
	// Compute values for d_output
    for (int n = 0; n < N; ++n) {
        int idx = n;
        const int i1 = idx % 10;     // First index for n_weight and nd_preact
        idx /= 10;
        const int i2 = idx % 6;      // Second index for n_weight and d_output
        idx /= 6;
        const int i3 = idx % 6;      // Third index for n_weight and d_output
        idx /= 6;
        const int i4 = idx;          // Fourth index for n_weight and d_output
...
void cpu_bp_preact_s1(float d_preact[6][6][6], float d_output[6][6][6], float preact[6][6][6]) {
    ...
    for (int n = 0; n < N; ++n) {
        int idx = n;
        const int i1 = idx % 6;    // First index
        idx /= 6;
        const int i2 = idx % 6;    // Second index
        idx /= 6;
        const int i3 = idx;        // Third index

        const float o = cpu_step_function(preact[i1][i2][i3]);
        d_preact[i1][i2][i3] = d_output[i1][i2][i3] * o * (1 - o);
    }
}
...
void cpu_bp_avgpool_s1(float d_output[6][24][24], float nd_preact[6][6][6]) {
	...
	for (int n = 0; n < N; ++n)
	{
		...
		d_output[i4][i5 * 4 + i2][i6 * 4 + i3] += nd_preact[i4][i5][i6];
	}
}
